...
import { Button, Chip, Divider, IconButton, Menu, Surface, Title } from "react-native-paper";
...
const ReviewsScreen = ({ navigation, route }: RootStackScreenProps<'ReviewsScreen'>) => {
  ...
  const {
    // data, 
    error, loading, fetchMore } = useGetWaterbodyReviews({
    id: params.waterbody,
    limit: LIMIT,
    offset: 0,
  ...
  const data = useGetWaterbodyReviewsMock(11)
      ...
      <Surface style={styles.heading}>
          <View style={globalStyles.frsb}>
              ...
              <Title style={{ fontWeight: '500'}}>
                  {params.title} {params.total !== undefined && `(${params.total} reviews)`}
              </Title>
          </View>
          <IconButton 
            size={28}
